...
if defined PY (
  ...
  REM ping is used as the delay rather than `timeout`, which resolves to the
REM  wrong program when this file is run from a unix-style shell.
  start "" /b cmd /c "ping -n 3 127.0.0.1 >nul & start "" http://localhost:%PORT%/fleck.html"
  %PY% serve.py %PORT%
  goto :eof
)
